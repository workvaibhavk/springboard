...
export default function Page() {
  ...
  >({});
  const [moduleProgressCache, setModuleProgressCache] = useState<
  ...
  const toggleCourseExpand = async (courseId: string) => {
    ...
    if (nowExpanded && !moduleProgressCache[courseId]) {
      ...
      console.log(
        `[toggleCourseExpand] Fetching module progress — userId=${selectedUser} courseId=${courseId}`,
      );
      try {
        ...
        console.log(
          `[toggleCourseExpand] Response status: ${response.status} ${response.statusText}`,
        );
        ...
        if (!response.ok) {
          ...
          console.log(
            `[toggleCourseExpand] ✅ Got ${data.moduleCompletions?.length} completions | totalModules=${data.totalModules} | completedCount=${data.completedCount}`,
          );
          if (data.moduleCompletions?.length === 0) {
            console.warn(
              "[toggleCourseExpand] ⚠️ Empty array returned — check server terminal for schema probe output",
            );
          }
  ...
  // If no timezone indicator is present, force it to UTC by adding 'Z'
          ...
          <thead>
            <tr>
              {[
                ...
                <th
                  key={h}
                  className="py-3 px-4 bg-gray-100 text-left text-sm font-semibold text-gray-700 uppercase tracking-wider"
                >
                  {h}
                </th>
              ))}
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 gap-3 text-sm mb-3">
                      ...
                      <div>
                        ...
                        <p className="text-gray-700">
                          {formatDuration(
                            enrollment.courses?.total_duration_seconds,
                          )}
                        </p>
                      </div>
                    </div>
                    ...
                    {isExpanded && (
                      <div className="mb-3 border border-gray-100 rounded-lg overflow-hidden">
                        {isLoadingMods ? (
                          ...
                          progress.moduleCompletions.length > 0 ? (
                          <table className="w-full text-sm">
                            ...
                            <tbody>
                              {progress.moduleCompletions
                                .sort(
                                  (a, b) =>
                                    (a.modules?.order_index ?? 0) -
                                    (b.modules?.order_index ?? 0),
                                )
                                .map((mod, idx) => (
                                  <tr
                                    key={mod.id}
                                    className={`border-b border-gray-50 ${mod.completed ? "bg-green-50/40" : "bg-white"}`}
                                  >
                                    ...
                                    <td className="py-2 px-3 text-gray-600 text-xs">
                                      {mod.completed_at
                                        ? formatDateTime(mod.completed_at)
                                        : "—"}
                                    </td>
                                    <td className="py-2 px-3 text-gray-600 text-xs">
